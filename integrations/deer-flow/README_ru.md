...
> [!NOTE]
> **DeerFlow 2.0 — проект переписан с нуля.** Общего кода с v1 нет. Оригинальный Deep Research фреймворк сохранён в ветке [`1.x`](https://github.com/bytedance/deer-flow/tree/main-1.x), а активная разработка идёт в 2.0.
...
> [!TIP]
> На Linux при ошибке `permission denied` для Docker daemon добавьте пользователя в группу `docker` и перелогиньтесь.
...
Регрессионные тесты находятся в `backend/tests/`.
